 ...
 * @author  WP Sunday
 * @license GPL-2.0+
 * @link    http://www.wpsunday.com/plugins/wp-sunday-events/
 */

/**
 * WP Sunday Featured Events widget class.
 *
 * @since 0.9.0
 *
 * @package WP Sunday\Widgets
 */
class WP_Sunday_Featured_Events extends WP_Widget {
	...
	function __construct() {
		...
		$widget_ops = array(
			'classname'   => 'featured-content featured-event',
			'description' => __( 'Displays featured events with thumbnails', 'wp-sunday' ),
		);

		$control_ops = array(
			'id_base' => 'featured-event',
		);

		parent::__construct( 'featured-event', __( 'WP Sunday - Events', 'wp-sunday' ), $widget_ops, $control_ops );
	...
	function widget( $args, $instance ) {
		...
		if ( ! empty( $instance['posts_cat'] ) )
			$query_args['tax_query'] = array(
        		array(
          			'taxonomy'  => 'wp_sunday_events_category',
          			'field'     => 'term_id',
		...
		if ( have_posts() ) : while ( have_posts() ) : the_post();
			...
			if ( $instance['show_title'] )
				...
				if ( ! empty( $instance['show_title'] ) ) {
					...
					/**
					 * Filter the featured events widget title.
					 *
					 * @since  0.9.0
					 *
					 * @param string $title    Featured event title.
					 * @param array  $instance {
					 *     Widget settings for this instance.
					 *
					 *     @type string $title                   Widget title.
					 *     @type int    $posts_cat               ID of the post category.
					 *     @type int    $posts_num               Number of events to show.
					 ...
					 *     @type bool   $show_title              True if featured event title should
					 *                                           be shown, false otherwise.
					 *     @type bool   $show_content            True if featured event content
	...
	function form( $instance ) {
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'posts_cat' ); ?>"><?php _e( 'Events Category', 'wp-sunday' ); ?>:</label>
			<?php
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'posts_num' ) ); ?>"><?php _e( 'Number of Events to Show', 'wp-sunday' ); ?>:</label>
		...
		<p>
			...
			<label for="<?php echo esc_attr( $this->get_field_id( 'show_title' ) ); ?>"><?php _e( 'Show Event Title', 'wp-sunday' ); ?></label>
		</p>
